...
export const Conditions: {[cond: string]: Condition} = Object.freeze({
	...
	furlong: immediate({
		filterEq(regions: RegionList, n: number, _0: CourseData, _1: HorseParameters, extra: RaceParameters) {
			const bounds = new Region(200 * n, 200 * n + 200);
			return regions.rmap(r => r.intersect(bounds));
		}
	}),
	...
	// NB. upstream flags this as "not properly implemented for xtoproad unique"
	...
	// GLOBAL BUNDLE: evaluated against the OTHER runner (see RaceParameters.otherHorse)
	running_style_count_nige_otherself: valueFilter(
		(_: CourseData, _h: HorseParameters, extra: RaceParameters) => +StrategyHelpers.strategyMatches(extra.otherHorse.strategy, Strategy.Nige)
	),
	running_style_count_senko_otherself: valueFilter(
		(_: CourseData, _h: HorseParameters, extra: RaceParameters) => +StrategyHelpers.strategyMatches(extra.otherHorse.strategy, Strategy.Senkou)
	),
	running_style_count_sashi_otherself: valueFilter(
		(_: CourseData, _h: HorseParameters, extra: RaceParameters) => +StrategyHelpers.strategyMatches(extra.otherHorse.strategy, Strategy.Sasi)
	),
	running_style_count_oikomi_otherself: valueFilter(
		(_: CourseData, _h: HorseParameters, extra: RaceParameters) => +StrategyHelpers.strategyMatches(extra.otherHorse.strategy, Strategy.Oikomi)
	),
