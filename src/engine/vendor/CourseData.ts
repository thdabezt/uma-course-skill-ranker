...
export namespace CourseHelpers {
	...
	export function getCourse(courseId: number): CourseData {
		...
		if (!isSortedByStart(course.slopes)) (course.slopes as CourseData['slopes'][number][]).sort((a,b) => a.start - b.start);
		Object.keys(course).forEach(k => Object.freeze(course[k as keyof CourseData]));
