...
export interface HpPolicy {
	...
	/** GLOBAL BUNDLE: absolute HP left (callers compare against 0). */
	remainingHp(): number
...
export class GameHpPolicy implements HpPolicy {
	...
	gutsModifier!: number
	/** GLOBAL BUNDLE: spot-struggle HP multipliers [normal, while rushed]; oonige pays far more. */
	competeTopModifier!: readonly [number, number]
	...
	getStatusModifier(state: {isPaceDown: boolean, isDownhillMode: boolean, isItidoriarasoi: boolean, isKakari: boolean}) {
		...
		// GLOBAL BUNDLE: while competing for the lead the rush multiplier is folded into the
		// spot-struggle table instead of stacking with it.
	...
	hpPerSecond(state: HpStatus, velocity: number) {
	...
	getLastSpurtPair(state: RaceState, maxSpeed: number, baseTargetSpeed2: number) {
		...
		const lastleg: HpStatus = {phase: 2 as Phase, isPaceDown: false, isDownhillMode: false, isItidoriarasoi: false, isKakari: false};
